...
struct Encoder : public Common
{
    ...
    // Один scrambler на весь поток пакетов в рамках одной TX-сессии
    ...
    {
        return 2; // payload length uint16 LE
    }
    ...
    void encode_frame(const uint8_t* payload, int payload_size)
    {
        ...
        // data[] = [len_lo][len_hi][payload...][padding]
...
template <typename T>
struct MemoryPCMWriter : public DSP::WritePCM<T>
{
    std::vector<T> buffer;
    int sample_rate_;
    int channels_;
    ...
    MemoryPCMWriter(int sample_rate, int channels)
        : sample_rate_(sample_rate), channels_(channels), ok_(true)
    {
    }
    ...
    void silence(int count) override
    {
        buffer.insert(buffer.end(), count * channels_, T(0));
    }
    ...
    void write(const T* data, int count, int channels) override
    {
        ...
        buffer.insert(buffer.end(), data, data + count * channels);
    }
...
template <typename value, typename cmplx, int rate>
struct EncoderSessionImpl
{
    MemoryPCMWriter<value> mem;
    Encoder<value, cmplx, rate> enc;

    EncoderSessionImpl(const EncodeConfig& cfg, int64_t call_sign, int oper_mode)
        : mem(cfg.sample_rate, cfg.channels)
        , enc(&mem, cfg.center_freq_hz, call_sign, oper_mode)
    {
    }

    int max_payload_bytes() const
    {
        return enc.max_payload_bytes();
    }

    std::vector<value> encode_chunk(const std::vector<uint8_t>& payload)
    {
        mem.clear();
        enc.encode_frame(payload.data(), static_cast<int>(payload.size()));
        enc.finish();
        return mem.buffer;
    }
};
...
template <typename value, typename cmplx, int rate>
static int max_payload_bytes_for_config_typed(const EncodeConfig& cfg, int64_t call_sign, int oper_mode)
{
    MemoryPCMWriter<value> mem(cfg.sample_rate, cfg.channels);
    Encoder<value, cmplx, rate> enc(&mem, cfg.center_freq_hz, call_sign, oper_mode);
    return enc.max_payload_bytes();
}
...
int max_payload_bytes_for_config(const EncodeConfig& cfg)
{
    ...
    int64_t call_sign = base40_encoder(cfg.callsign.c_str());
    if (call_sign <= 0 || call_sign >= 262144000000000L)
        throw std::runtime_error("Unsupported call sign.");

    int oper_mode = 0;
    std::string error;
    if (!build_oper_mode(cfg, oper_mode, error))
    ...
    {
    case 44100:
        return max_payload_bytes_for_config_typed<value, cmplx, 44100>(cfg, call_sign, oper_mode);
    case 48000:
        return max_payload_bytes_for_config_typed<value, cmplx, 48000>(cfg, call_sign, oper_mode);
    default:
        throw std::runtime_error("Unsupported sample rate.");
    }
}
...
EncoderSessionHandle create_encoder_session(const EncodeConfig& cfg)
{
    ...
    {
        h.impl = new EncoderSessionImpl<value, cmplx, 44100>(cfg, call_sign, oper_mode);
    }
    else
    {
        h.impl = new EncoderSessionImpl<value, cmplx, 48000>(cfg, call_sign, oper_mode);
    }
...
void destroy_encoder_session(EncoderSessionHandle& h)
{
    ...
    if (h.sample_rate == 44100)
        delete static_cast<EncoderSessionImpl<value, cmplx, 44100>*>(h.impl);
    else if (h.sample_rate == 48000)
        delete static_cast<EncoderSessionImpl<value, cmplx, 48000>*>(h.impl);
...
{
    EncodeChunkResult result{};
    result.ok = false;

    if (!h.impl)
    {
        result.error = "Encoder session is not initialized.";
        return result;
    }
    ...
    try
    {
        if (h.sample_rate == 44100)
        {
            auto* enc = static_cast<EncoderSessionImpl<value, cmplx, 44100>*>(h.impl);
            result.pcm = enc->encode_chunk(payload);
        }
        else if (h.sample_rate == 48000)
        {
            auto* enc = static_cast<EncoderSessionImpl<value, cmplx, 48000>*>(h.impl);
            result.pcm = enc->encode_chunk(payload);
        }
        else
        {
